...
uint8_t BMP280::readValues(double *_temperature, double *_pressure)
{
  ...
  if(I2C::readBurstRegValue(BMP280_DEVICE_ADDR, BMP280_REGISTER_TEMPDATA, 3, &_tmp) == 0) return 0;  
  adc_T = (int32_t)_tmp >> 4;
  
  // retreive temperature
  if(I2C::readBurstRegValue(BMP280_DEVICE_ADDR, BMP280_REGISTER_PRESSUREDATA , 3, &_tmp) == 0) return 0;
...
{
  return -(double)(R_PERFECT_GAZ * _T / (G_0 * M_AIR)) * log(_P/this->P0);
}
...
*     - Status (uint8_t) : 1 success, 0 error
**/
uint8_t BMP280::setReferenceAltitude()
{
  double P, T;
