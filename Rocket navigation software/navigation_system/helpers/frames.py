...
from . import quaternion_helper

# sea level gravitational constant
G0 = 9.81

# WGS 84 geometrical parameters
EQUATORIAL_RADIUS = 6378137.0 # m - equatorial radius, a
POLAR_RADIUS = 6356752.314245 # m - polar radius, b
...
def geodetic_to_ecef(longitude, latitude, altitude, deg=False):
	...
	f = POLAR_RADIUS/EQUATORIAL_RADIUS
	e = np.sqrt(1 - f**2)
	N = EQUATORIAL_RADIUS/np.sqrt(1 - e**2 * np.sin(latitude)**2)
...
def ecef_to_geodetic(ecef, deg=False):
	...
	p = np.sqrt(x**2 + y**2)
	F = 54.0*POLAR_RADIUS**2*z**2
	G = p**2 + (1-e2)*z**2 - e2*(EQUATORIAL_RADIUS**2-POLAR_RADIUS**2)
	...
	r0 = -P*e2*p/(1+Q) + np.sqrt(0.5*EQUATORIAL_RADIUS**2*(1+1/Q) - P*(1-e2)*z**2/(Q*(1+Q)) - 0.5*P*p**2)
	del P, Q
	...
	z0 = POLAR_RADIUS**2*z/(EQUATORIAL_RADIUS*V)
	altitude = U * (1-POLAR_RADIUS**2/(EQUATORIAL_RADIUS*V))
	del U, V

	ep2 = (EQUATORIAL_RADIUS**2 - POLAR_RADIUS**2)/POLAR_RADIUS**2
