...
def kalman_gps(X, angular_vel, accel, gps, mag, g, dt):   
    ...
    accel_RF = accel + np.dot(np.linalg.inv(M_rot), g)

    F = state_transition_model_function_jacobian(X, accel_RF, dt)
    
    # predict
    x_prediction = state_transition_model_function(X, accel_RF, angular_vel, dt, M_rot)
    P_prediction = np.dot(F, np.dot(P, F.transpose())) + Q
    
    # update
    z = measurement_vector_gps(gps, angular_vel, mag)
...
def measurement_vector_gps(gps, w, mag):
    return np.array([*gps, *w, *mag, *(np.random.rand(3) - 0.5)*0.001])
...
def state_transition_model_function(x, a, w, dt, M_rot):
    # Kalman variables
    # x                             x[0]
    # y                             x[1]
    # z                             x[2]
    ...
    # inputs : a, w imu measurements

    x_new = x.copy()
 
    x_new[0:3] += np.dot(M_rot, (x[6:9] + (a - x[18:21]) * dt/2) * dt) # x y z (in enu frame)
    x_new[3:6] += np.dot(M_rot, (a - x[18:21]) * dt) # roll pitch yaw (in enu frame)
    x_new[6:9] += (x[9:12]- x[15:18]) * dt # vx vy vz (in rocket frame)

    return x_new
         
def state_transition_model_function_jacobian(x, a, dt):
    global n_dim
    ...
    # x[0] x
    F[0][3] = ((cr*sp*cy + sy*sr) * (x[7] + (a[1] - x[19])*dt/2) + (cr*sy - sp*cy*sr)*(x[8] + (a[2] - x[20])*dt/2)) * dt
    F[0][4] = (-cy*sp * (x[6] + (a[0] - x[18])*dt/2) + sr*cp*cy * (x[7] + (a[1] - x[19])*dt/2) + cp*cy*cr*(x[8] + (a[2] - x[20])*dt/2)) * dt
    F[0][5] = (-sy*cp * (x[6] + (a[0] - x[18])*dt/2) - (sr*sp*sy + cy*cr) * (x[7] + (a[1] - x[19])*dt/2) + (sr*cy - sp*sy*cr)*(x[8] + (a[2] - x[20])*dt/2)) * dt
    ...
    # x[1] y
    F[1][3] = ((cr*sp*sy - cy*sr)*(x[7] + (a[1] - x[19])*dt/2) - (cr*cy + sp*sy*sr)*(x[8] + (a[2] - x[20])*dt/2))*dt
    F[1][4] = (-sp*sy*(x[6] + (a[0] - x[18])*dt/2) + sr*cp*sy*(x[7] + (a[1] - x[19])*dt/2) + cp*sy*cr*(x[8] + (a[2] - x[20])*dt/2))*dt
    F[1][5] = (cp*cy*(x[6] + (a[0] - x[18])*dt/2) + (sr*sp*cy - sy*cr)*(x[7] + (a[1] - x[19])*dt/2) + (sr*sy+sp*cy*cr)*(x[8] + (a[2] - x[20])*dt/2))*dt
    ...
    # x[2] z
    F[2][3] = (cr*cp*(x[7] + (a[1] - x[19])*dt/2) - cp*sr*(x[8] + (a[2] - x[20])*dt/2))*dt
    F[2][4] = (-cp*(x[6] + (a[0] - x[18])*dt/2) - sr*sp*(x[7] + (a[1] - x[19])*dt/2) - sp*cr*(x[8] + (a[2] - x[20]) * dt/2))*dt
    ...
    # x[3] angx
    F[3][3] = ((cr*sp*cy + sy*sr)*(x[10] - x[16]) + (cr*sy - sp*cy*sr)*(x[11] - x[17]))*dt
    F[3][4] = (-cy*sp*(x[9] - x[15]) + sr*cp*cy*(x[10] - x[16]) + cp*cy*cr*(x[11] - x[17]))*dt
    F[3][5] = (-sy*cp*(x[9] - x[15]) + -(sr*sp*sy + cy*cr)*(x[10] - x[16]) + (sr*cy - sp*sy*cr)*(x[11] - x[17]))*dt
    
    F[3][18] = -cy*cp * dt
    F[3][19] = -(sr*sp*cy - sy*cr) * dt
    F[3][20] = -(sr*sy + sp*cy*cr) * dt
    
    # x[4] angy
    F[4][3] = ((cr*sp*sy - cy*sr)*(x[10] - x[16]) - (cr*cy + sp*sy*sr)*(x[11] - x[17]))*dt
    F[4][4] = (-sp*sy*(x[9] - x[15]) + sr*cp*sy*(x[10] - x[16]) + cp*sy*cr*(x[11] - x[17]))*dt
    F[4][5] = (cp*cy*(x[9] - x[15]) + (sr*sp*cy - sy*cr)*(x[10] - x[16]) + (sr*sy + sp*cy*cr)*(x[11] - x[17]))*dt
    
    F[4][18] = -cp*sy*dt
    F[4][19] = -(sr*sp*sy + cy*cr)*dt
    F[4][20] = -(-sr*cy+sp*sy*cr)*dt
    
    # x[5] angz  
    F[5][3] = (cr*cp*(x[10] - x[16]) - cp*sr*(x[11] - x[17]))*dt
    F[5][4] = -(cp*(x[9] - x[15]) + sr*sp*(x[10] - x[16]) + sp*cr*(x[11] - x[17]))*dt
    F[5][5] = 0
...
def main(arduino):
    ...
    Q = np.zeros(n_dim)
    R = np.diag([*s_pos, *s_vel, *s_w, *s_mag, *s_pos]) # noise variance  
    
    gravity = np.array([0, 0, -1]) * 9.81
    
    state_vector[15:18] = m_w * np.pi / 180.0
    state_vector[18:21] = m_a * 9.81 + gravity
    
    state_vector[12:15] = m_mag

    P = np.random.randn(n_dim, n_dim)*0.1
    ...
    legend = ["x", "y", "z", "angx", "angy", "angz", "vx", "vy", "vz", "wx", "wy", "wz", "mx", "my", "mz", "bwx", "bwy", "bwz", "bax", "bay", "baz", "bmx", "bmy", "bmz"]   
