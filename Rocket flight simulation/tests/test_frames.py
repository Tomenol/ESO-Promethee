...
print(frames.geodetic_to_ecef(lon, lat, alt, deg=True))

ecef = frames.geodetic_to_ecef(lon, lat, alt, deg=True)

print(frames.ecef_to_geodetic(ecef, deg=False))
print(frames.ecef_to_geodetic(ecef, deg=True))

ecef_1 = np.array([15.5, 22, 32.5])
print(ecef_1)
enu = frames.ecef_to_enu(ecef_1, ecef, only_rotation=True)
print(enu)
print(frames.enu_to_ecef(enu, ecef, only_rotation=True))
...
pad = rocket_simulator.LaunchPad(10, 45, 0.1, env, initial_attitude_euler=initial_rpy, deg=True, logger=logging)
...
t_law 		= np.array([0.0, 0.060, 0.280, 0.5000, 1.0000, 3.0000, 3.59])
...
drag_coefficient_body 			= 1.5

lift_rate_coefficient_tail 		= 2*np.pi
drag_coefficient_tail 			= 0.1
...
basis_ecef = np.eye(3)
basis_pad = frames.ecef_to_enu(np.eye(3), pad.get_launch_position_ecef(), only_rotation=True)
basis_rocket = np.dot(quaternion.Quaternion(*pad.get_launch_attitude()).DCM(), basis_pad)

fig=plt.figure()
ax=fig.add_subplot(111, projection="3d")

for i in range(3):
	ax.plot([0, basis_ecef[i, 0]], [0, basis_ecef[i, 1]], [0, basis_ecef[i, 2]], "-k")
for i in range(3):
	ax.plot([0, basis_pad[i, 0]], [0, basis_pad[i, 1]], [0, basis_pad[i, 2]], "--b")
for i in range(3):
	ax.plot([0, basis_rocket[i, 0]], [0, basis_rocket[i, 1]], [0, basis_rocket[i, 2]], ":r")
plt.show()